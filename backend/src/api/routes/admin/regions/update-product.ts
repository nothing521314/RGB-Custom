...
export default async (req, res) => {
    const {region_id, product_id} = await validator(
        AdminPostRegionsRegionCountriesReq,
        req.params
    )

    const {price} = await validator(AdminPostRegionsProductReq, req.body)
    ...
    await manager.transaction(async (transactionManager) => {
        return await regionService
            ...
            .updateProduct(region_id, product_id, price)
    })
...
export class AdminPostRegionsProductReq {
    @IsNumber()
    price: number
}
