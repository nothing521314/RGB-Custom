...
import {ProductAdditionalRepository} from "../repositories/product-additional-hardware";
...
type InjectedDependencies = {
    ...
    productAdditionalHardwareRepository: typeof ProductAdditionalRepository
}
...
class ProductService extends TransactionBaseService {
    ...
    protected readonly productAdditionalRepository_: typeof ProductAdditionalRepository
    ...
    constructor({
                    ...
                    productRepository,
                    productAdditionalHardwareRepository,
        ...
        this.productAdditionalRepository_ = productAdditionalHardwareRepository
    ...
    async createHardware(product: string, hardware: string) {
        return await this.atomicPhase_(async (manager) => {
            ...
            const add = await productAddRepo.findOne({
                where: {
                    product_parent_id: product,
                    product_additions_id: hardware
                }
            })
            ...
            if (!add) {
                ...
                await productAddRepo.save(additional)
            }
        })
    }
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            try {
                ...
                let product = productRepo.create({
                    ...rest,
                    prices: priceList,
                    additional_hardwares: additionalHardwareList
                })
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            if (prices) {
                await Promise.all(prices.map(async item => {
                    const productPrice = await productPriceRepo.findOne({
                        where: {
